...
const print = async ({ printInfo }) => {
  ...
  const result = `User ${userId} printed ${pageCount} pages on printer ${printerId} with ${
    config.color
  } color, ${config.pageType} page type, ${
    config.duplex ? "duplex" : "simplex"
  } and ${config.printCount} copies and paid ${paymentAmount}VN$`;
  console.log(result);
  return result;
};
